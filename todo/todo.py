...
def index():
    db, c=get_db()
    c.execute("SELECT t.id, t.description, u.username, t.completed, t.created_at "
...
def create():
    if request.method=='POST':
        ...
        error=None
        if not description: error="Descripcion es requerida"
...
def get_todo(id):
    ...
    todo=c.fetchone()
    if todo is None: abort(404, "El todo de id {0} no existe".format(id))
    return todo
...
def update(id):
    ...
    if request.method=='POST':
        ...
        error=None
        if not description: error="La descripcion es requerida"
        if error is not None: flash(error)
        else:
            db, c=get_db()
            c.execute(
